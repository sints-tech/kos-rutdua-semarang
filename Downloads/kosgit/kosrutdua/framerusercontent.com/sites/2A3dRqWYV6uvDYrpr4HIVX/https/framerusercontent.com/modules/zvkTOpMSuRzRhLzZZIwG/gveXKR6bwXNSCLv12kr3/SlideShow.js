...
import {
    usePageVisibility
} from "https://framerusercontent.com/modules/V9ryrjN5Am9WM1dJeyyJ/GzHgU466IQmt8g4qOKj8/UsePageVisibility.js";
...
function awaitRefCallback(element, controller) {
    ...
    const refCallbackPromise = new Promise((resolve, reject) => {
        ...
        controller.signal.addEventListener("abort", () => reject); // resolve immediately if it exists
        const current = element.current;
        if (current) resolve(current);
    }).catch(() => {}); // we need to listen to the ref setter, so let's override `current` - we can do that, because we don't use React's `useRef` hook for those refs.
...
export default function Slideshow(props) {
    ...
    const scheduleMeasure = useCallback(async () => {
        ...
        if (!isCanvas && (!firstChild.current || !lastChild.current)) try {
            await Promise.all([awaitRefCallback(firstChild, controller), awaitRefCallback(lastChild, controller)]);
        } catch {
            controller.abort();
        }
        frame.read(measure);
