    ...
    try {
        ...
        var getPath = function(overwrite) {
            ...
            if (isFunction(pathOverwriterFunction)) {
                try {
                    path = pathOverwriterFunction.call(window, {
                        path: path
                    }) || path;
                } catch (error) {
        ...
        var pageview = function(isPushState, pathOverwrite, metadata) {
            ...
            userNavigated = performaceEntryType ?
                ["reload", "back_forward"].indexOf(performaceEntryType) > -1 : // Check if back, forward or reload buttons are being use in older browsers
                // 1: TYPE_RELOAD, 2: TYPE_BACK_FORWARD
                perf &&
                perf[navigationText] && [1, 2].indexOf(perf[navigationText].type) > -1;
