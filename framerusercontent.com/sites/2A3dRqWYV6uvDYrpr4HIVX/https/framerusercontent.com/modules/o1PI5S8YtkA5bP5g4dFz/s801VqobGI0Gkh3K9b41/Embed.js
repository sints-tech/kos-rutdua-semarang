...
function EmbedUrl({
    ...
    const hasAutoHeight = !style.height; // Add https:// if the URL does not have a protocol.
    if (!/[a-z]+:\/\//.test(url)) {
        url = "https://" + url;
    }
    ...
    useEffect(() => { // We only want to check on the canvas.
...
function EmbedHtml({
    ...
    if (hasScript) {
        ...
        if (hasSplineViewer || hasComment) {
            return /*#__PURE__*/ _jsx(EmbedHtmlWithScripts, {
                html: html,
                ...props
            });
        }
        return /*#__PURE__*/ _jsx(EmbedHtmlInsideIframe, {
            html: html,
            ...props
        });
    }
    return /*#__PURE__*/ _jsx(EmbedHtmlWithoutScripts, {
        html: html,
        ...props
    });
}
...
function EmbedHtmlInsideIframe({
    ...
    const ref = useRef();
    const [iframeHeight, setIframeHeight] = useState(0); // Handle auto sizing
    useEffect(() => {
        ...
        window.addEventListener("message", handleMessage); // After SSG the iframe loads before we attach the event handler,
...
`;
    const currentStyle = { ...iframeStyle,
        ...style
    };
